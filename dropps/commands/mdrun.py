...
def mdrun(args):
    ...
    parameters = loaded_params["parameters"]
    mdsystem = loaded_params["mdsystem"]
    mdtopology = loaded_params["mdtopology"]
    positions = loaded_params["positions"]
    pdb_raw = loaded_params["pdb_raw"]
    ...
    if args.checkpoint is not None:
        print(f"## WARNING: You have specified a checkpoint file.")
        print(f"## Thus, the simulation will be performed without energy minimization and warming.")

    if parameters["minimize"] is True and args.checkpoint is None:
    ...
    reporter_xtc.report(simulation, simulation.context.getState(getPositions=True, getVelocities=True))

    # Warming
    if temperature_initial < temperature_final and args.checkpoint is None:
    ...
    wrapped_positions = positions % box_vec  # modulo operation wraps coordinates

    x_nms = [position[0] for position in wrapped_positions]
    y_nms = [position[1] for position in wrapped_positions]
    z_nms = [position[2] for position in wrapped_positions]
...
def getargs_mdrun(argv):
    ...
    parser.add_argument('-s', '--run-input', type=str, required=True, 
                        help="TPR file containing all information for this simulation run.")
    parser.add_argument('-cpi', '--checkpoint', type=str, required=False,
                        help="Checkpoint file to initialize the simulation.")
